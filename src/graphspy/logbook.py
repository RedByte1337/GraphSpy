...
def _make_formatter(utc: bool = False):
    """Return a format function configured for UTC or local time."""
    ...
    def _format_message(record):
        ...
        symbols = {
            ...
            "WARNING": (
                f"<fg {warning_orange}>[!]</fg {warning_orange}>",
                warning_orange,
            ),
        ...
        if utc:
            time_fmt = "{time:YYYY-MM-DD HH:mm:ss.SSS!UTC} (UTC)"
        else:
            time_fmt = "{time:YYYY-MM-DD HH:mm:ss.SSS} (UTC{time:Z})"

        return (
            f"<fg {time_gray}>{time_fmt}</fg {time_gray}> "
            f"{symbol} "
...
def setup_logging(level: str = "INFO", log_dir: Path | None = None, utc: bool = False):
    """
    ...
    Args:
        ...
        utc: If True, display timestamps in UTC. Otherwise use local time with UTC offset.
    """
    level = level.upper()
    ...
    logger.add(
        ...
        diagnose=True,
        level=level,
        format=_make_formatter(utc=utc),
        colorize=True,
    )
    ...
    if utc:
        ...
        file_format = (
            "{time:YYYY-MM-DD HH:mm:ss.SSS} (UTC{time:Z}) "
            "[{level:7}] {message}\n"
            "{exception}"
        )
